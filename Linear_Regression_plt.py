...
def getLine(xRange,theta):
    ...
    ****************************************
    ...
    """
    return plt.Line2D(xRange,[xRange[0]*theta[0,1]+theta[0,0],xRange[1]*theta[0,1]+theta[0,0]])
...
def computeCost(X,y,theta):   
    ...
    ****************************************
    ...
    """ 
    return 1.0/(2*X.shape[1])*np.sum(np.power(theta*X-y.T,2))
...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    for i in range(0, num_iters):
        theta = theta - alpha / m * (np.dot(theta*X-y.T,X.T))
...
YPoint = []
[YPoint.append(t[0,0]+Xrange[0]*t[0,1]) or YPoint.append(t[0,0]+Xrange[1]*t[0,1]) 
    for t in theta_history] #it just get all the y in f(x)
data = np.array([Xrange * iterations,YPoint])
l, = plt.plot([], [], 'b-',linewidth=3)
plt.title('Regression data(Auto)')
line_ani = animation.FuncAnimation(fig2, update_line, 50, fargs=(data, l),interval=500,blit=True)
...
for i in range(0,len(theta0_vals)):
    for j in range(0,len(theta1_vals)):
        ...
        J_vals[i,j] = computeCost(np.matrix([np.ones(m), X]), y, t)
